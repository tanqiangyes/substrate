...
pub enum DigestItem {
	...
	/// 这些是从共识引擎到运行时的消息，尽管共识引擎可以（并且应该）自己读取它们以避免代码和状态重复。
	/// 运行时生成这些是错误的，但这（尚未）检查。
	/// NOTE: the runtime is not allowed to panic or fail in an `on_initialize`
	/// call if an expected `PreRuntime` digest is not present. It is the
	/// responsibility of a external block verifier to check this. Runtime API calls
	/// will initialize the block without pre-runtime digests, so initialization
	/// cannot fail when they are missing.
	/// 注意：如果不存在预期的“PreRuntime”摘要，则不允许运行时在“on_initialize”调用中出现恐慌或失败。
	/// 外部区块验证者有责任对此进行检查。运行时 API 调用将在没有运行前摘要的情况下初始化块，因此在缺少它们时初始化不会失败。
	PreRuntime(ConsensusEngineId, Vec<u8>),

	/// A message from the runtime to the consensus engine. This should *never*
	/// be generated by the native code of any consensus engine, but this is not
	/// checked (yet).
	/// 从运行时到共识引擎的消息。这不应该由任何共识引擎的本机代码生成，但尚未检查（目前）。
	Consensus(ConsensusEngineId, Vec<u8>),

	/// Put a Seal on it. This is only used by native code, and is never seen
	/// by runtimes.
	/// 盖上印章。这仅由本机代码使用，运行时永远不会看到。
...
/// 摘要项目的“参考视图”。不拥有其内容。由最终运行时实现用于对其日志项进行编码解码。（跟DigestItem相似，但是只是持有其引用）
#[derive(PartialEq, Eq, Clone, RuntimeDebug)]
...
/// 摘要项目的类型。用于获得对“DigestItem”编码过程的显式控制。
/// 我们需要一个显式控制，因为最终运行时使用 `DigestItemRef` 类型对它们自己的摘要项进行编码，
/// 并且我们不能为 `DigestItemRef` 自动派生 `Decode` 特征。
#[repr(u32)]
...
/// 包含原始数据的摘要项目的类型；这还会在适用的情况下命名共识引擎 ID。用于识别一个或多个感兴趣的摘要项目。
...
impl DigestItem {
	...
	/// 如果 `Some` 如果此条目具有给定的 id，则返回该条目中包含的不透明数据。
	...
	/// 如果此条目具有给定的 id，则返回项目中包含的数据 if `Some`，解码为提供的类型 `T`。
...
impl<'a> DigestItemRef<'a> {
	...
	/// 尝试将此摘要项与给定的不透明项标识符匹配；如果匹配，则返回它包含的不透明数据。
	...
	/// 尝试将此摘要项与给定的不透明项标识符匹配；如果匹配，则尝试转换为给定的数据类型；如果可行，返回它。
	pub fn try_to<T: Decode>(&self, id: OpaqueDigestItemId) -> Option<T> {
