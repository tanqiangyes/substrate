...
//! 内在的外在因素是指固有地添加到每个块中的外在因素。然而，这取决于运行时的实现，是否要求每个块都有一个固有的，或者使之成为可选的。
//! 继承因子主要用于将数据从块生产者传递给运行时。因此，内在物需要一些运行在客户端的部分和一些运行在运行时端的部分。
//! 构建继承体时，继承体所需的任何数据都会作为[`InherentData`]从客户端传递给运行时。
